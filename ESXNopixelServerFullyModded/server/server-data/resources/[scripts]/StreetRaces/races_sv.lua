        ...
        for index, race in pairs(races) do
            ...
            elseif (race.finishTime ~= 0) and (time > race.finishTime + race.finishTimeout) then
                ...
                for _, player in pairs(players) do
                    TriggerClientEvent('DoLongHudText',player, "DNF (timeout)", 2)
                end
...
AddEventHandler("races:createRace_sv", function(amount, startDelay, startCoords, checkpoints, currentMap, finishTimeout)
    ...
    local index = #races
    TriggerClientEvent("races:createRace_cl", -1, index, amount, startDelay, startCoords, checkpoints, currentMap)
end)
    ...
    for index, race in pairs(races) do
        ...
        if source == race.owner and time < race.startTime then
            ...
            for _, player in pairs(race.players) do
                ...
                TriggerClientEvent('DoLongHudText',player, msg, 2)
            end
...
AddEventHandler("races:joinRace_sv", function(index)
    ...
    if playerMoney >= amount then
        ...
        TriggerClientEvent('DoLongHudText',source, msg, 2)
    end
end)
            ...
            if race.finishTime == 0 then
                ...
                for _, pSource in pairs(players) do
                    if pSource == source then
                        ...
                        TriggerClientEvent('DoLongHudText',pSource, msg,3)
