...
Citizen.CreateThread(function()
	...
	while true do
			...
			if dstscan < 2.5 then
				...
				if IsPedInAnyVehicle(PlayerPedId(), true) == false and dstscan < 0.25 then
					...
					if IsControlJustPressed(1, 38) then	
						DBWF.TriggerServerCallback('dbfw-license:checkLicense', function(hasWeaponLicense)
		...
		-- 		if IsControlJustPressed(1, 38) then	
		-- 			local finished = exports["dbfw-taskbar"]:taskBar(60000,"Readying Smelter")
...
----------Issa400 end
    	...
    	if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
			...
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
				...
				if IsControlJustPressed(1, 38) then
					local finished = exports["dbfw-taskbar"]:taskBar(60000,"Searching...")
    	...
    	if(Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 10.0)then
			...
			if(Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 2.0)then
				...
				if IsControlJustPressed(1, 38) and (Vdist(1775.6893310547,2593.6455078125,45.723571777344, pos.x, pos.y, pos.z) < 2.0) then
					local finished = exports["dbfw-taskbar"]:taskBar(60000,"Making a god slushy...")
      				if (finished == 100) then
		...
		if(Vdist(487.2354, -997.1319, 30.68965, pos.x, pos.y, pos.z) < 10.0)then
			found = true
			local job = DBWF.GetPlayerData().job.name
			if job == "police" then
			DrawMarker(27, 487.2354, -997.1319, 30.68965 - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 0, 25, 165, 165, 0,0, 0,0)
			if(Vdist(487.2354, -997.1319, 30.68965, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Press ~INPUT_CONTEXT~ Open Junk Locker")
				if IsControlJustPressed(1, 38) and (Vdist(487.2354, -997.1319, 30.68965, pos.x, pos.y, pos.z) < 2.0) then
