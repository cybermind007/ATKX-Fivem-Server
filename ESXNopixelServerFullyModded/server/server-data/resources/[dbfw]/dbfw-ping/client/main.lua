    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('dbfw-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('dbfw-ping:client:SendPing')
AddEventHandler('dbfw-ping:client:SendPing', function(sender, senderId)
    if pendingPing == nil then
        ...
        TriggerServerEvent('dbfw-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('dbfw-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('dbfw-ping:client:AcceptPing')
AddEventHandler('dbfw-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('dbfw-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('dbfw-ping:client:RejectPing')
AddEventHandler('dbfw-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('dbfw-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('dbfw-ping:client:RemovePing')
AddEventHandler('dbfw-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
