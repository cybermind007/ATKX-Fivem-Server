Citizen.CreateThread(function()
	Citizen.Wait(500)
	print('\27[32m[dbfw-core]\27[0m Loaded All Data Successfully')
end)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
			...
			table.insert(tasks2, function(cb2)
				...
				MySQL.Async.fetchAll('SELECT job, job_grade, loadout, position FROM users WHERE identifier = @identifier', {
					...
					if DBFWCore.DoesJobExist(job, grade) then
						...
						print(('dbfw-core: %s had an unknown job [job: %s, grade: %s], setting as unemployed!'):format(player.getIdentifier(), job, grade))
		...
		Async.parallel(tasks, function(results)
			...
			xPlayer.getMissingAccounts(function(missingAccounts)
				...
				DBFWCore.Players[_source] = xPlayer

				TriggerEvent('dbfw:playerLoaded', _source, xPlayer)

				TriggerClientEvent('dbfw:playerLoaded', _source, {
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		TriggerEvent('dbfw:playerDropped', _source, reason)
		TriggerEvent('dbfw:PlayerDropped', xPlayer, source)

		DBFWCore.SavePlayer(xPlayer, function()
			DBFWCore.Players[_source] = nil
			DBFWCore.LastPlayerData[_source] = nil
		end)
	end
end)

RegisterServerEvent('dbfw:updateLoadout')
AddEventHandler('dbfw:updateLoadout', function(loadout)
	local xPlayer = DBFWCore.GetPlayerFromId(source)
	xPlayer.loadout = loadout
end)

RegisterServerEvent('dbfw:updateLastPosition')
AddEventHandler('dbfw:updateLastPosition', function(position)
	local xPlayer = DBFWCore.GetPlayerFromId(source)
	xPlayer.setLastPosition(position)
end)

DBFWCore.RegisterServerCallback('dbfw:getPlayerData', function(source, cb)
	local xPlayer = DBFWCore.GetPlayerFromId(source)
...
DBFWCore.RegisterServerCallback('dbfw:getOtherPlayerData', function(source, cb, target)
...
DBFWCore.StartDBSync()
DBFWCore.StartPayCheck()
