...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('dbfw-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('dbfw-hospital:client:SyncBleed')
AddEventHandler('dbfw-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('dbfw-hospital:client:FieldTreatLimbs')
AddEventHandler('dbfw-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('dbfw-hospital:client:FieldTreatBleed')
AddEventHandler('dbfw-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('dbfw-hospital:client:ReduceBleed')
AddEventHandler('dbfw-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('dbfw-hospital:client:ResetLimbs')
AddEventHandler('dbfw-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('dbfw-hospital:client:RemoveBleed')
AddEventHandler('dbfw-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('dbfw-hospital:client:UsePainKiller')
AddEventHandler('dbfw-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
...
RegisterNetEvent('dbfw-hospital:client:UseAdrenaline')
AddEventHandler('dbfw-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end
