...
function spectate(target)
	...
	DBFWCore.TriggerServerCallback('dbfw:getPlayerData', function(player)
...
function getPlayersList()
	...
	local players = DBFWCore.Game.GetPlayers()
	local data = {}
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(1, 163) then
			if group ~= "user" then
				TriggerEvent('dbfw-spectate:spectate')
			end
		end
	end
end)
...
RegisterNetEvent('dbfw-spectate:spectate')
AddEventHandler('dbfw-spectate:spectate', function()
...
RegisterNUICallback('kick', function(data, cb)
	SetNuiFocus(false)
	TriggerServerEvent('dbfw-spectate:kick', data.id, data.reason)
	TriggerEvent('dbfw-spectate:spectate')
end)
