...
RegisterServerEvent('dbfw-identity:setIdentity')
AddEventHandler('dbfw-identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('dbfw-identity:identityCheck', myIdentifiers.playerid, true)
		else
...
AddEventHandler('es:playerLoaded', function(source)
	...
	TriggerClientEvent('dbfw-identity:saveID', source, myID)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('dbfw-identity:identityCheck', source, false)
			TriggerClientEvent('dbfw-identity:showRegisterIdentity', source)
		else
			TriggerClientEvent('dbfw-identity:identityCheck', source, true)
		end
	end)
end)
...
AddEventHandler('onResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		local xPlayers, xPlayer = DBFWCore.GetPlayers()
		for i=1, #xPlayers, 1 do
			xPlayer = DBFWCore.GetPlayerFromId(xPlayers[i])
			...
			TriggerClientEvent('dbfw-identity:saveID', xPlayer.source, myID)

			getIdentity(xPlayer.source, function(data)
				if data.firstname == '' then
					TriggerClientEvent('dbfw-identity:identityCheck', xPlayer.source, false)
					TriggerClientEvent('dbfw-identity:showRegisterIdentity', xPlayer.source)
				else
					TriggerClientEvent('dbfw-identity:identityCheck', xPlayer.source, true)
				end
			end)
		end
	end
