...
RegisterNUICallback('btnTaskGroups', function()
    local jobgrade = DBFWCore.PlayerData.job.grade
    local rankConverted = DBFWCore.PlayerData.job.grade_label

    local groupObject = {}
    table.insert(groupObject, {
        namesent = DBFWCore.PlayerData.job.label,
...
function findVehFromPlateAndLocate(plate)
  ...
  local gameVehicles = DBFWCore.Game.GetVehicles()
...
RegisterNUICallback('btnAttempCreateHouse', function(data, cb)
  TriggerEvent("dbfw-housing:client:createHouses", data.price, data.tier)
  loadedHouse = false
...
function hasPhone()
  ...
  if
    (
    (exports["dbfw-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["dbfw-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["dbfw-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["dbfw-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["dbfw-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["dbfw-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsDisabledControlJustReleased(1, 199) then
      TriggerEvent("dbfw:heHasPhone")
    end
  end
end)

RegisterNetEvent('dbfw:heHasPhone')
AddEventHandler('dbfw:heHasPhone', function()
    GotPhone()
end)
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    if DBFWCore.PlayerData.job.name == "realestateagent" then
     isREAgent = true
    end
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  if DBFWCore.PlayerData.job.name ~= "taxi" and DBFWCore.PlayerData.job.name ~= "towtruck" and DBFWCore.PlayerData.job.name ~= "trucker" then
    TriggerServerEvent('phone:updatePhoneJob', data.advert)
  else
    TriggerServerEvent('phone:updatePhoneJob', DBFWCore.PlayerData.job.name .. " | " .. data.advert)
...
AddEventHandler('getAccountInfo', function(cash, bank, licences)
    local responseObject = {
        cash = cash,
        bank = bank,
        job = DBFWCore.PlayerData.job.name .. ', ' .. DBFWCore.PlayerData.job.grade_label,
...
RegisterNetEvent("dbfw-phone:userTwitter")

AddEventHandler("dbfw-phone:userTwitter", function(user)
  userTwitter = user
 end)
