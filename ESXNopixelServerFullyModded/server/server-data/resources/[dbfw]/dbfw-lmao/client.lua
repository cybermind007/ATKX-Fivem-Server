...
RegisterNUICallback("ban", function(data,cb)
	...
	DBFWCore.TriggerServerCallback("el_bwh:ban",function(success,reason)
		if success then DBFWCore.ShowNotification("~g~Successfully banned player") else DBFWCore.ShowNotification(reason) end -- dont ask why i did it this way, im a bit retarded
...
RegisterNUICallback("warn", function(data,cb)
	...
	DBFWCore.TriggerServerCallback("el_bwh:warn",function(success)
		if success then DBFWCore.ShowNotification("~g~Successfully warned player") else DBFWCore.ShowNotification("~r~Something went wrong") end
	end, data.target, data.message, data.anon)
end)

RegisterNUICallback("unban", function(data,cb)
	if not data.id then return end
	DBFWCore.TriggerServerCallback("el_bwh:unban",function(success)
		if success then DBFWCore.ShowNotification("~g~Successfully unbanned player") else DBFWCore.ShowNotification("~r~Something went wrong") end
	end, data.id)
end)

RegisterNUICallback("getListData", function(data,cb)
	if not data.list or not data.page then cb(nil); return end
	DBFWCore.TriggerServerCallback("el_bwh:getListData",function(data)
		cb(data)
...
AddEventHandler("el_bwh:acceptedAssist",function(t)
	...
	if target then
		...
		assisting = true
		assist_target = t
		DBFWCore.Game.Teleport(ped,GetEntityCoords(GetPlayerPed(target))+vector3(0,0.5,0))
	end
end)
...
AddEventHandler("el_bwh:assistDone",function()
	if assisting then
		assisting = false
		if pos_before_assist~=nil then DBFWCore.Game.Teleport(GetPlayerPed(-1),pos_before_assist+vector3(0,0.5,0)); pos_before_assist = nil end
		assist_target = nil
	end
end)
...
AddEventHandler("el_bwh:showWindow",function(win)
	...
	elseif win=="banlist" or win=="warnlist" then
		...
		DBFWCore.TriggerServerCallback(win=="banlist" and "el_bwh:getBanList" or "el_bwh:getWarnList",function(list,pages)
		...
		while true do
			...
			if IsControlJustPressed(0, 117) then
				if not last_assist then
					DBFWCore.ShowNotification("~r~Noone requested assistance yet")
				elseif not NetworkIsPlayerActive(GetPlayerFromServerId(last_assist)) then
					DBFWCore.ShowNotification("~r~The player that requested assistance is not online anymore")
					last_assist=nil
				else
