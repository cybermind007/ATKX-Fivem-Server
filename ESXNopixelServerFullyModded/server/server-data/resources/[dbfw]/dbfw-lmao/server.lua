...
Citizen.CreateThread(function() -- startup
    ...
    while DBFWCore==nil do Wait(0) end
    ...
    DBFWCore.RegisterServerCallback("el_bwh:ban", function(source,cb,target,reason,length,offline)
        if not target or not reason then return end
        local xPlayer = DBFWCore.GetPlayerFromId(source)
        local xTarget = DBFWCore.GetPlayerFromId(target)
    ...
    DBFWCore.RegisterServerCallback("el_bwh:warn",function(source,cb,target,message,anon)
    ...
    DBFWCore.RegisterServerCallback("el_bwh:getWarnList",function(source,cb)
        local xPlayer = DBFWCore.GetPlayerFromId(source)
    ...
    DBFWCore.RegisterServerCallback("el_bwh:getBanList",function(source,cb)
        local xPlayer = DBFWCore.GetPlayerFromId(source)
        -- local start = GetGameTimer() -- debug
        if isAdmin(xPlayer) then
    ...
    DBFWCore.RegisterServerCallback("el_bwh:getListData",function(source,cb,list,page)
        local xPlayer = DBFWCore.GetPlayerFromId(source)
        if isAdmin(xPlayer) then
    ...
    DBFWCore.RegisterServerCallback("el_bwh:unban",function(source,cb,id)
        local xPlayer = DBFWCore.GetPlayerFromId(source)
        if isAdmin(xPlayer) then
...
function logAdmin(msg)
    for k,v in ipairs(DBFWCore.GetPlayers()) do
        if isAdmin(DBFWCore.GetPlayerFromId(v)) then
...
function banPlayer(xPlayer,xTarget,reason,length,offline)
    ...
    MySQL.Async.execute("INSERT INTO bwh_bans(id,receiver,sender,length,reason) VALUES(NULL,@receiver,@sender,@length,@reason)",{["@receiver"]=json.encode(targetidentifiers),["@sender"]=xPlayer.identifier,["@length"]=length,["@reason"]=reason},function(_)
        ...
        if offline then xTarget = DBFWCore.GetPlayerFromIdentifier(xTarget) end -- just in case the player is on the server, you never know
        if xTarget then
...
AddEventHandler("el_bwh:acceptAssistKey",function(target)
    ...
    acceptAssist(DBFWCore.GetPlayerFromId(_source),target)
end)
