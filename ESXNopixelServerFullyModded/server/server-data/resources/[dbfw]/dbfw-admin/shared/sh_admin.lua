...
dbfw._Admin.Ranks = {}
dbfw._Admin.Commands = {}
dbfw._Admin.Players = {}
dbfw._Admin.DiscPlayers = {}
dbfw._Admin.CurAdmins = {}

function dbfw.Admin.Log(self, log, user)
...
function dbfw.Admin.ChatPrint(self, msg, target)

end

function dbfw.Admin.RankExists(self, rank)
    rank = tostring(rank)
...
function dbfw.Admin.CommandExists(self, cmd)
    cmd = tostring(cmd)
...
function dbfw.Admin.RankInheritsCommand(self, rank, cmd)
...
function dbfw.Admin.GetPlayerRank(self, user)
    if not IsDuplicityVersion() then
        return group
    end
end

function dbfw.Admin.RankHasCommand(self, rank, cmd)
...
function dbfw.Admin.GetRanks(self)
    return dbfw._Admin.Ranks
end

function dbfw.Admin.GetRankData(self, rank)
    if not rank then return false end
...
function dbfw.Admin.GetCommands(self)
    return dbfw._Admin.Commands
end

function dbfw.Admin.GetCategories(self)
    local tmp = {}
...
function dbfw.Admin.GetCommandData(self, cmd)
    if not cmd then return false end
...
function dbfw.Admin.IsAdmin(self, user)
...
function dbfw.Admin.IsSuperAdmin(self, user)
    if not IsDuplicityVersion() then
...
function dbfw.Admin.IsValidUser(self, user)
    if not user then return false end
...
function dbfw.Admin.CanTarget(self, user, target)
...
function dbfw.Admin.CanTargetRank(self, user, rank)
...
function dbfw.Admin.AddCommand(self, cmd)
    local tmp = {}
    ...
    if chicken then
        ...
        tmp.runcommand = function(args) args.command = tmp.command dbfw.Admin:RunCommand(args) end
...
function dbfw.Admin.AddRank(self, rank, data)
    local tmp = {}
...
function dbfw.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        if not player then else dbfw._Admin.Players[src].status = status end

        for k,v in pairs(dbfw._Admin.CurAdmins) do
            TriggerClientEvent("dbfw-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("dbfw-admin:setStatus", status)
end

AddEventHandler("np-base:exportsReady", function()
    exports["np-base"]:addModule("Admin", dbfw.Admin)
    exports["np-base"]:addModule("_Admin", dbfw._Admin)
end)