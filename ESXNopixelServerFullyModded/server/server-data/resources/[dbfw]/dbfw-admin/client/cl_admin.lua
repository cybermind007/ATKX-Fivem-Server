...
function dbfw.Admin.CheckForSessions(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local players = dbfw._Admin.Players

            for k,v in pairs(players) do
                local src = v.src
                local playerId = GetPlayerFromServerId(src)
                if not src then
                    dbfw._Admin.Players[src].sessioned = true
                else
                    if not NetworkIsPlayerActive(playerId) or not NetworkIsPlayerConnected(playerId) then dbfw._Admin.Players[src].sessioned = true end
                end
            end
        end
    end)
...
stop = true
function dbfw.Admin.BeginAFK(self, stop)
    if stop then
        afk.isAfk = false
        afk.msgAFK = false
        afk.stringToType = ""
    ...
    afk.event = AddEventHandler("dbfw-admin:afkStringCheck", function(text)
        if string.lower(text) == afk.stringToType then dbfw.Admin:BeginAFK(true) return end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("dbfw-admin:Disconnect", "AFK Kick") return end
        end
    end)
end

function dbfw.Admin.AFKCheck(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local rank = dbfw.Admin:GetPlayerRank()
            local rankData = dbfw.Admin:GetRankData(rank)
...
RegisterNetEvent("dbfw-admin:afkStringCheck")

RegisterNetEvent("dbfw-admin:setStatus")
AddEventHandler("dbfw-admin:setStatus", function(src, status)
    local player = dbfw._Admin.Players[src]
    if not player then return else dbfw._Admin.Players[src].status = status end
end)

RegisterNetEvent("dbfw-admin:sendPlayerInfo")
AddEventHandler("dbfw-admin:sendPlayerInfo", function(data, discData)
    dbfw._Admin.Players = data
    dbfw._Admin.DiscPlayers = discData
end)

RegisterNetEvent("dbfw-admin:RemovePlayer")
AddEventHandler("dbfw-admin:RemovePlayer", function(data)
    dbfw._Admin.DiscPlayers[data.src] = data
    dbfw._Admin.Players[data.src] = nil
end)

RegisterNetEvent("dbfw-admin:RemoveRecent")
AddEventHandler("dbfw-admin:RemoveRecent", function(data)
    if data ~= nil then
        dbfw._Admin.DiscPlayers[data.src] = nil
    end
end)

RegisterNetEvent("dbfw-admin:AddPlayer")
AddEventHandler("dbfw-admin:AddPlayer", function(player)
    dbfw._Admin.Players[player.src] = player
end)

RegisterNetEvent('dbfw-admin:teleportUser')
AddEventHandler('dbfw-admin:teleportUser', function(x, y, z)
    SetEntityCoords(PlayerPedId(), x, y, z)
    TriggerEvent('DoLongHudText', 'You were summoned')
end)

RegisterNetEvent("dbfw-admin:AddAllPlayers")
AddEventHandler("dbfw-admin:AddAllPlayers", function(data)
    dbfw._Admin.Players[data.src] = data
end)

function dbfw.Admin.RunCommand(self, args)
    if not args or not args.command then return end
    TriggerServerEvent("dbfw-admin:runCommand", args)
end

function dbfw.Admin.RunClCommand(self, cmd, args)
    if not cmd or not self:CommandExists(cmd) then return end
    self:GetCommandData(cmd).runclcommand(args)
end

RegisterNetEvent("dbfw-admin:RunClCommand")
AddEventHandler("dbfw-admin:RunClCommand", function(cmd, args)
    dbfw.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("dbfw-admin:updateData")
AddEventHandler("dbfw-admin:updateData", function(src, type, data)
    if not src or not type or not data then return end
    if not dbfw._Admin.Players[src] then return end
    
    dbfw._Admin.Players[src][type] = data
end)

RegisterNetEvent("dbfw-admin:noLongerAdmin")
AddEventHandler("dbfw-admin:noLongerAdmin", function()
    dbfw._Admin.Players = {}
    
    for k,v in pairs(dbfw._Admin.Menu.Menus) do
        if WarMenu.IsMenuOpened(k) then WarMenu.CloseMenu() end
    end
end)

RegisterNetEvent("dbfw-admin:bringPlayer")
AddEventHandler("dbfw-admin:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("dbfw-admin:bring")
AddEventHandler("dbfw-admin:bring", function(target)
    ...
    TriggerServerEvent("dbfw-admin:bringPlayerServer",pos,target)   
end)



 
RegisterNetEvent("dbfw-admin:runSpawnCommand")
AddEventHandler("dbfw-admin:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
function dbfw.Admin.teleportMarker(self)
...
function dbfw.Admin.GetBanTimeFromString(self, time)
...
RegisterNetEvent("dbfw-admin:ReviveInDistance")
AddEventHandler("dbfw-admin:ReviveInDistance", function()
    local playerList = {}
