...
RegisterNetEvent("dbfw-housing:client:decorate")
AddEventHandler("dbfw-housing:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
...
RegisterNetEvent("dbfw-housing:server:sethousedecorations")
AddEventHandler("dbfw-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		DBFWCore.TriggerServerCallback('dbfw-housing:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("dbfw-housing:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		DBFWCore.ShowNotification('Speed is '.. tostring(speeds[curSpeed]))
	end
