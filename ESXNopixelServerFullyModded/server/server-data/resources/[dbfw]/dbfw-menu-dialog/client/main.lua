Citizen.CreateThread(function()
	...
	local openMenu = function(namespace, name, data)
		for i=1, #Timeouts, 1 do
			DBFWCore.ClearTimeout(Timeouts[i])
		end
		...
		local timeoutId = DBFWCore.SetTimeout(200, function()
	...
	DBFWCore.UI.Menu.RegisterType(MenuType, openMenu, closeMenu)

	RegisterNUICallback('menu_submit', function(data, cb)
		local menu = DBFWCore.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
		local post = true
		...
		if menu.submit ~= nil then
			...
			if tonumber(data.value) ~= nil then
				...
				-- Round float values
				data.value = DBFWCore.Math.Round(tonumber(data.value))
