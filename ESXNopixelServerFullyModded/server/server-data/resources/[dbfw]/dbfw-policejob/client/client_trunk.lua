    ...
    if not DoesVehicleHaveDoor(veh, 6) and DoesVehicleHaveDoor(veh, 5) and IsThisModelACar(GetEntityModel(veh)) then
       ...
       -- TriggerEvent("animation:PlayAnimation","trunk")
        while intrunk do
            ...
            --print('im player that in trunk?', intrunk, TrunkedPlate)
...
AddEventHandler('ped:releaseTrunkCheck', function()
    ...
    if curplate then
        t, distance = GetClosestPlayer()
        TriggerServerEvent("ped:trunkAccepted",curplate, GetPlayerServerId(t))
    end
...
AddEventHandler('ped:releaseTrunk', function(licensePlate)
    ...
    -- print(licensePlate .. " | " .. curplate)
    -- print(licensePlate == curplate)
   -- if licensePlate == curplate then
        TriggerEvent("ped:intrunk",false)
        SetVehicleDoorOpen(trunkveh, 5, 1, 1)
        trunkveh = 0
        TrunkedPlate = 0
...
RegisterCommand("myjobs", function(source, args)
	print(exports["isPed"]:isPed("myjob"))
	print(exports["isPed"]:isPed("cid"))
end)

Citizen.CreateThread(function()
	
	TriggerEvent('chat:addSuggestion', '/trunkin', 'Crawl in vehicle trunk!')
    ...
    TriggerEvent('chat:addSuggestion', '/a', 'to answer a call')
    TriggerEvent('chat:addSuggestion', '/answer', 'to answer a call')
    TriggerEvent('chat:addSuggestion', '/h', 'to hangup a call')
    TriggerEvent('chat:addSuggestion', '/hangup', 'to hangup a call')
    TriggerEvent('chat:addSuggestion', '/atm', 'to open atm')
    TriggerEvent('chat:addSuggestion', '/bank', 'to open bank')
