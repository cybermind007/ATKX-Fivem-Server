...
AddEventHandler('dbfw:playerLoaded', function(newhotSpots)
	blnPlySpawned = true
end)
--Strawberry
--Rancho
...
AddEventHandler('drugs:corner', function()
    ...
    if (MyStreetName == "Forum Dr" or MyStreetName == "Brouge Ave" or MyStreetName == "Grove St" or MyStreetName == "Macdonald St" or MyStreetName == "Jamestown St" or MyStreetName == "Carson Ave") and exports["dbfw-inventory"]:hasEnoughOfItem("weedq",1,false) and dst < 500.0 then
...
function SellDrugs(NPC,saleprice, amount)
	...
	local crack = exports["dbfw-inventory"]:hasEnoughOfItem("1gcrack",amount,false)
	if crack and sellingcrack then
		TriggerEvent("inventory:removeItem", "1gcrack", amount)
	end

	local weedbaggies = exports["dbfw-inventory"]:hasEnoughOfItem("weedq",amount,false)
	if weedbaggies and sellingweed then
		TriggerEvent("inventory:removeItem", "weedq", amount)
	end


	local cocaine = exports["dbfw-inventory"]:hasEnoughOfItem("1gcocaine",amount,false) 
