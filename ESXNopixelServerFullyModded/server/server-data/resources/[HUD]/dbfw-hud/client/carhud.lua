...
RegisterNetEvent('db-overallhud:client:ProximityActive')
AddEventHandler('db-overallhud:client:ProximityActive', function(active)
	tokovoipstate = active
end)
...
Citizen.CreateThread(function()
    while true do
		if IsPedInAnyVehicle(PlayerPedId()) and not currentValues["notveh"] then
			local time = CalculateTimeToDisplay()
			drawTxt(0.097, 0.97, 1.0,1.0,0.4 ,"" .. time.hour .. ":" .. time.minute, 255, 255, 255, 255)
    		...
    		local area_zone = zoneNames[tostring(zone)]
...
Citizen.CreateThread(function()
	while true do
		if currentValues["stress"] >= 25 then
			ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.02)
		elseif currentValues["stress"] >= 50 then
			ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.07)
		elseif currentValues["stress"] >= 75 then
...
AddEventHandler("RemoveOxyTank",function()
	if currentValues["oxy"] > 25.0 then
		GearAnim()
		-- DBFW.Functions.Progressbar("remove_gear", "Oxygen Tank", 10000, false, true, {}, {}, {}, {}, function() -- Done
			currentValues["oxy"] = 25.0
		...
		-- DBFW.Functions.Notify('You are not wearing an oxygen tank.', 'error')
	end
end)
...
AddEventHandler("UseOxygenTank",function()
	GearAnim()
	-- DBFW.Functions.Progressbar("equip_gear", "Oxygen Tank", 30000, false, true, {}, {}, {}, {}, function() -- Done
		ClearPedTasks(GetPlayerPed(-1))
		currentValues["oxy"] = 100.0
		TriggerEvent('menu:hasOxygenTank', true)
		TriggerServerEvent('oxygengear:RemoveGear')
		-- DBFW.Functions.Notify('/divingsuit to take off your diving suit!', 'error')
	-- end)
end)
...
AddEventHandler("fire:damageUser", function(Reqeuester)
	...
	if IsPedShooting(Attackerped) then
        ...
        if name == `WEAPON_FIREEXTINGUISHER` and not IsEntityDead(GetPlayerPed(-1)) then
    ...
    while true do
        if not IsGaining then
            ...
            if StressGain > 0 then
				TriggerEvent('notification', 'Stress gained.', 1)
                TriggerServerEvent('db-hud:Server:GainStress', math.random(20000, 50000))
                StressGain = 0
            end
        end
        Citizen.Wait(3000)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         local WeaponData = DBFW.Shared.Weapons[CurrentWeapon]
        ...
        if car ~= 0 and (wasInCar or IsCar(car)) then
            ...
            if IsControlJustPressed(0, 29) then
                if usingSeatbelt == false then
                    usingSeatbelt = true
					TriggerEvent('notification', 'Belt On.', 1)
                    TriggerEvent('InteractSound_CL:PlayOnOne', 'buckle', 0.8)
                else
                    usingSeatbelt = false
					TriggerEvent('notification', 'Belt Off.', 2)
...
Citizen.CreateThread(function()
	...
	currentValues["talking"] = tokovoipstate
	...
	while true do
		...
		if IsPedShooting(get_ped) then
			currentValues["stress"] = currentValues["stress"] + 5
		end
		...
		if counter == 0 then
			...
			currentValues["notveh"] = not IsPedInAnyVehicle(PlayerPedId())

			for k , v in pairs(Noradar) do
				local Van = GetVehiclePedIsIn(GetPlayerPed(-1), false)
				if IsVehicleModel(Van, GetHashKey(v)) then
					currentValues["notveh"] = true
				end
			end

			currentValues["belt"] = not usingSeatbelt
			currentValues["talking"] = tokovoipstate
			if tokovoipstate then
				currentValues["talking"] = 100
			else
				currentValues["talking"] = 0
			end
			
			if isLoggedIn then
				local lakhu = IsPauseMenuActive()
				if lakhu == 1 then
					SendNUIMessage({
						action = "hudtick",
						show = false,
				...
				TriggerEvent('dbfw_status:getStatus', 'hunger', function(hunger)
					TriggerEvent('dbfw_status:getStatus', 'thirst', function(thirst)
						TriggerEvent('dbfw_status:getStatus', 'stress', function(stress)
							...
							currentValues["hunger"] = myhunger
							currentValues["thirst"] = mythirst
							currentValues["stress"] = mystress
				...
				currentValues["proximity"] = 1
				-- exports["mumble-voip"]:GetPlayerRadioProxy(GetPlayerServerId(PlayerId()))
				currentValues["armor"] = GetPedArmour(GetPlayerPed(-1))
			 end
			currentValues["speed"] = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 2.23694
			currentValues["speed"] = math.ceil(currentValues["speed"])
			currentValues["direction"] = math.floor(calcHeading(-GetEntityHeading(player) % 360))
			currentValues["fuel"] = exports['LegacyFuel']:GetFuel(GetVehiclePedIsIn(GetPlayerPed(-1)))
			currentValues["engine"] = GetVehicleEngineHealth(GetVehiclePedIsIn(GetPlayerPed(-1)))
			...
			if valueChanged then
				SendNUIMessage({
					...
					varSeatBelt = currentValues["belt"],
					varTalking = currentValues["talking"],
					varSetFuel = currentValues["fuel"],
					varSetVoip = currentValues["proximity"],
					varSetSpeedProg = math.ceil(currentValues["speed"]),
					varSetOxy = currentValues["oxy"],
					varSetEngine = currentValues["engine"],
					vardirection = currentValues["direction"],
		...
		counter = counter - 1
			if get_ped_veh ~= 0 and not currentValues["notveh"] then
...
AddEventHandler('food:FishTaco', function()
	...
	TriggerEvent('dbfw_status:add','hunger', 800000)
	if currentValues["hunger"] < 0 then
		currentValues["hunger"] = 0
	end
...
AddEventHandler('food:Taco', function()
	currentValues["hunger"] = 100
	TriggerEvent('dbfw_status:add','hunger', 1000000)
...
AddEventHandler('food:Condiment', function()
	dstamina = 0
	Citizen.Wait(1000)
	TriggerEvent('dbfw_status:add','hunger', 400000)
...
AddEventHandler('changethirst', function()
	TriggerEvent('dbfw_status:add','thirst', 250000)
...
AddEventHandler('coffee:drink', function()
	TriggerEvent('dbfw_status:add','thirst', 450000)
...
AddEventHandler('changehunger', function()
	...
	TriggerEvent('dbfw_status:add','hunger', 250000)
        ...
        if vehModel ~= nil and vehModel ~= 0 then
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                	local finished = exports["dbfw-taskbar"]:taskBar(2000,"Toggling Anchor")
...
RegisterNetEvent("dbfw-jobmanager:playerBecameJob")
AddEventHandler("dbfw-jobmanager:playerBecameJob", function(job, name)
...
AddEventHandler("client:newStress",function(positive,alteredValue)
	...
	if positive then
		...
		TriggerServerEvent("db-hud:Server:GainStress", 2000)
	else
		TriggerEvent("DoShortHudText",'Stress Relieved',6)
		TriggerServerEvent("db-hud:Server:RelieveStress", 160000)
	end
